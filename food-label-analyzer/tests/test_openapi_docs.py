...
def test_openapi_routes_expose_summary_description_and_responses(monkeypatch) -> None:
    ...
    assert isinstance(report_detail.get("description"), str) and report_detail["description"]
    ...
    assert isinstance(analysis_upload.get("summary"), str) and analysis_upload["summary"]
    assert isinstance(analysis_upload.get("description"), str) and analysis_upload["description"]
    assert set(analysis_upload["responses"].keys()) >= {"200", "400", "401", "429", "503", "422"}


def test_openapi_schemas_expose_field_metadata_and_new_report_shape(monkeypatch) -> None:
    load_required_env(monkeypatch, SKIP_STARTUP_CHECKS="true")
    main_module = importlib.reload(importlib.import_module("app.main"))

    schema = main_module.app.openapi()
    report_detail_schema = schema["components"]["schemas"]["ApiResponse_ReportDetailResponseSchema_"]
